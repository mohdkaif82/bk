...
class SlackExceptionHandler(AdminEmailHandler):
    ...
    def emit(self, record, *args, **kwargs):
        if not settings.DEBUG:
            ...
            attachments = [
                {
                    ...
                    'fields': [
                        ...
                        {
                            "title": "User",
                            "value": ((request.user.mobile + ' (' + str(request.user.pk) + ')'
            ...
            send = True
            part1 = message.split("Django Version:")[0]
            traceback = message.split("Traceback:")
            try:
                main_error = traceback[1] if traceback and len(traceback) > 1 else "No Traceback"
                part2 = main_error.split("Request information:")[0]
                message = part1 + part2
            except:
                print("Error Occoured. Message was not filtered. Sending Original message.")
            if send:
                split = 7900
                ...
                webhook_url = 'https://hooks.slack.com/services/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'
